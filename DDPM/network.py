...
# 配置字典 : 定义不同配置的网络结构
convnet_small_cfg = { # 较小的 ConvNet 架构
    'type': 'ConvNet',
    'intermediate_channels': [10, 20],
    'pe_dim': 128
}

convnet_medium_cfg = { # 中等大小的 ConvNet 架构，带时间步编码插入
    'type': 'ConvNet',
    'intermediate_channels': [10, 10, 20, 20, 40, 40, 80, 80],
    'pe_dim': 256,
    'insert_t_to_all_layers': True
}
convnet_big_cfg = { # 较大的 ConvNet 架构，带时间步编码插入
    'type': 'ConvNet',
...
unet_1_cfg = { # 基本的 U-Net 架构
    'type': 'UNet',
    'channels': [10, 20, 40, 80],
      'pe_dim': 128
}

unet_res_cfg = { # 带残差连接的 U-Net 架构
    'type': 'UNet',
    'channels': [10, 20, 40, 80],
    'pe_dim': 128,
    'residual': True
...
def build_network(config: dict, n_steps):
    '''
    description:根据配置字典构建相应的网络
    param  [dict] config: 配置字典
    param  [*] n_steps: 扩散过程的时间步数
    return [*] 配置好的网络
    '''
